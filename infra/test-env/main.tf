...
provider "aws" {
  region  = "us-east-1"
  profile = "transilience-admin"

  default_tags {
    tags = {
      Project     = "transilience-test"
      Environment = "test"
      ManagedBy   = "terraform"
    }
  }
...
resource "aws_iam_user" "dev_no_mfa" {
  name = "dev-no-mfa"
  tags = {
    transilience_expected = "fail"
    transilience_check    = "iam-mfa-enabled"
  }
}
...
resource "aws_iam_user" "overprivileged" {
  ...
  tags = {
    transilience_expected = "fail"
    transilience_check    = "iam-least-privilege"
  }
}
...
resource "aws_iam_user" "stale_keys_user" {
  ...
  tags = {
    transilience_expected = "fail"
    transilience_check    = "iam-access-key-rotation"
  }
}
...
resource "aws_s3_bucket" "insecure" {
  bucket = "transilience-test-insecure-${data.aws_caller_identity.current.account_id}"
  tags = {
    transilience_expected = "fail"
    transilience_check    = "s3-encryption s3-versioning s3-public-access"
  }
}

# GOOD: S3 bucket with encryption, versioning, and public access blocked
resource "aws_s3_bucket" "secure" {
  bucket = "transilience-test-secure-${data.aws_caller_identity.current.account_id}"
  tags = {
    transilience_expected = "pass"
    transilience_check    = "s3-encryption s3-versioning s3-public-access"
  }
...
resource "aws_vpc" "test" {
  ...
  tags = { Name = "transilience-test-vpc" }
}
...
resource "aws_subnet" "public" {
  ...
  tags = { Name = "transilience-test-public" }
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone = "us-east-1a"
  tags = { Name = "transilience-test-private" }
}

resource "aws_internet_gateway" "test" {
  vpc_id = aws_vpc.test.id
  tags   = { Name = "transilience-test-igw" }
}
...
resource "aws_route_table" "public" {
  ...
  tags = { Name = "transilience-test-public-rt" }
}
...
resource "aws_security_group" "bad_all" {
  name        = "transilience-bad-all-traffic"
...
resource "aws_s3_bucket" "cloudtrail" {
  bucket = "transilience-cloudtrail-${data.aws_caller_identity.current.account_id}"
  tags   = { Name = "transilience-cloudtrail-logs" }
}
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "aws:SourceArn" = "arn:aws:cloudtrail:us-east-1:${data.aws_caller_identity.current.account_id}:trail/transilience-trail"
          }
        }
      },
      {
...
resource "aws_cloudtrail" "main" {
  name                       = "transilience-trail"
  s3_bucket_name             = aws_s3_bucket.cloudtrail.id
  is_multi_region_trail      = true
  enable_log_file_validation = true
  include_global_service_events = true

  tags = { Name = "transilience-trail" }
...
resource "aws_guardduty_detector" "main" {
  ...
  tags = { Name = "transilience-guardduty" }
}
...
resource "aws_s3_bucket" "config" {
  bucket = "transilience-config-${data.aws_caller_identity.current.account_id}"
  tags   = { Name = "transilience-config-logs" }
}
...
resource "aws_iam_role_policy" "config_s3" {
  name = "transilience-config-s3-delivery"
...
resource "aws_config_configuration_recorder" "main" {
  name     = "transilience-config-recorder"
...
resource "aws_config_delivery_channel" "main" {
  name           = "transilience-config-channel"
...
output "summary" {
  value = <<-EOT
    ...
    Transilience Test Environment Created!
    ================================
    ...
    Networking (CC6.6):
      - VPC: ${aws_vpc.test.id}
      - transilience-bad-ssh          → SSH open to 0.0.0.0/0 (FAIL)
      - transilience-bad-rdp          → RDP open to 0.0.0.0/0 (FAIL)
      - transilience-bad-all-traffic  → all ports open to 0.0.0.0/0 (FAIL)
      - transilience-good-web         → HTTPS from restricted CIDR (PASS)
      - No VPC flow logs (FAIL)

    Logging & Monitoring (CC7.1, CC7.2, CC8.1):
      - CloudTrail: transilience-trail (multi-region, log validation) (PASS)
