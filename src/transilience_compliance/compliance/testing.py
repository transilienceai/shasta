...
Transforms automated checks into structured "control tests" that
...
from transilience_compliance.evidence.models import ComplianceStatus, Finding, ScanResult
...
class ControlTest:
    ...
    remediation: str = ""
    tested_at: str = ""
    tested_by: str = "Transilience Automated Testing"
...
def save_control_test_report(suite: ControlTestSuite, output_path: Path | str = "data/reports") -> Path:
    ...
    lines = [
        ...
        f"**Tested By:** Transilience Automated Compliance Testing",
        "",
        "## Summary",
        "",
        f"| Result | Count |",
    ...
    for t in suite.tests:
        ...
        if t.evidence:
            ...
            for e in t.evidence:
                ...
                lines.append(f"| `{e['resource'][-50:]}` | {e['status']} | {e['severity']} | {desc} |")
            lines.append("")
    ...
    lines.extend([
        ...
        "",
        "*This report was generated by Transilience Automated Compliance Testing. All tests were performed programmatically against the live AWS environment. Evidence artifacts are available in the evidence store for detailed review.*",
    ])
