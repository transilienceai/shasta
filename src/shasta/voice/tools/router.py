...
def list_findings(req: ListFindingsReq, request: Request):
    return _timed(
        ...
        lambda: findings_tool.list_findings(
            store=_store(request), **req.model_dump(exclude_none=True)
        ),
    )
...
def get_resource_findings(req: IdReq, request: Request):
    return _timed(
        ...
        lambda: findings_tool.get_resource_findings(
            store=_store(request), resource_id=req.resource_id or ""
        ),
    )
...
def get_score_trend(req: GetScoreTrendReq, request: Request):
    return _timed(
        "get_score_trend",
        req.model_dump(),
        lambda: scores_tool.get_score_trend(
            store=_store(request), framework=req.framework, limit=req.limit
        ),
    )
...
def get_control_summary(req: GetControlSummaryReq, request: Request):
    return _timed(
        "get_control_summary",
        req.model_dump(exclude_none=True),
        lambda: controls_tool.get_control_summary(
            store=_store(request), framework=req.framework, control_id=req.control_id
        ),
    )
...
def get_latest_scan(request: Request):
    return _timed("get_latest_scan", {}, lambda: scans_tool.get_latest_scan(store=_store(request)))
...
def list_risk_items(req: ListRisksReq, request: Request):
    return _timed(
        "list_risk_items",
        req.model_dump(exclude_none=True),
        lambda: risks_tool.list_risk_items(
            store=_store(request), account_id=req.account_id, status=req.status, level=req.level
        ),
    )
...
def get_risk_item(req: GetRiskReq, request: Request):
    return _timed(
        "get_risk_item",
        req.model_dump(),
        lambda: risks_tool.get_risk_item(
            store=_store(request), risk_id=req.risk_id, account_id=req.account_id
        ),
    )
...
def add_risk_item(req: AddRiskReq, request: Request):
    return _timed(
        "add_risk_item",
        req.model_dump(exclude_none=True),
        lambda: risks_tool.add_risk_item(
            store=_store(request), **req.model_dump(exclude_none=True)
        ),
    )
