...
def list_risk_items(
    *, store: Store, account_id: str, status: str | None = None, level: str | None = None
) -> list[dict[str, Any]]:
    return [
        r.model_dump(mode="json")
        for r in store.list_risk_items(account_id, status=status, level=level)
    ]


def get_risk_item(
    *, store: Store, risk_id: str, account_id: str = "123456789012"
) -> dict[str, Any]:
