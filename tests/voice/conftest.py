...
def seeded_db_path(tmp_path: Path) -> Path:
    ...
    findings = [
        ...
        _make_finding(
            id="f-003",
            check_id="azure-sql-tls",
            title="SQL TLS below 1.2",
            severity=Severity.CRITICAL,
            status=ComplianceStatus.FAIL,
            domain=CheckDomain.ENCRYPTION,
            resource_id="/subscriptions/x/sql/y",
        ...
        _make_finding(
            ...
            title="CloudTrail disabled in audit account",
            severity=Severity.CRITICAL,
            status=ComplianceStatus.FAIL,
            domain=CheckDomain.LOGGING,
            resource_id="arn:aws:cloudtrail::123:trail/audit",
            soc2=["CC7.1", "CC7.2"],
        ),
        # 3 high
        _make_finding(
            ...
            title="Stale IAM key >180d",
            severity=Severity.HIGH,
            status=ComplianceStatus.FAIL,
            domain=CheckDomain.IAM,
            resource_id="arn:aws:iam::123:user/legacy-bot",
            soc2=["CC6.3"],
        ),
        _make_finding(
            id="f-011",
            check_id="sg-open-22",
            title="Security group open on 22",
            severity=Severity.HIGH,
            status=ComplianceStatus.FAIL,
            domain=CheckDomain.NETWORKING,
            resource_id="sg-0e1f2a3b",
            soc2=["CC6.6"],
        ),
        _make_finding(
            id="f-012",
            check_id="lambda-perm-role",
            title="Lambda overly permissive role",
            severity=Severity.HIGH,
            status=ComplianceStatus.FAIL,
            domain=CheckDomain.IAM,
            resource_id="arn:aws:lambda::123:function/proc",
            soc2=["CC6.1"],
        ),
        ...
        _make_finding(
            ...
            title="S3 versioning off",
            severity=Severity.MEDIUM,
            status=ComplianceStatus.FAIL,
            domain=CheckDomain.STORAGE,
            resource_id="arn:aws:s3:::dev-build",
        ),
        _make_finding(
            id="f-021",
            check_id="ebs-encryption-default",
