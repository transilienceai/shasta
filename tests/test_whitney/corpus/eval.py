...
Usage:
    ...
    py -3.12 -m tests.test_whitney.corpus.eval --json out.json
...
from tests.test_whitney.corpus.loader import CORPUS_ROOT, Fixture, load_corpus
...
def run_eval(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # Phase C alpha: apply guardrail suppression inside the tempdir scope
        # so relative file_path resolves correctly against the scan root.
        try:
            from whitney.code.dataflow import suppress_guardrailed_findings

            findings, _suppressed = suppress_guardrailed_findings(
                findings, scan_root=tmp_root
            )
        except ImportError:
            pass  # dataflow module not yet available; run without suppression
